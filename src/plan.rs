...
pub fn remote_divergence(pushes: &[Push], is_local: impl Fn(&str) -> bool) -> Vec<&Push> {
    pushes
        .iter()
        .filter(|p| {
            p.expected_remote_sha
                .as_deref()
                .is_some_and(|sha| !is_local(sha))
        })
        .collect()
}

pub fn divergence_warning(
    diverged: &[&Push],
    remote: &str,
    top_branch: &str,
    overwriting: bool,
    ...
    let list: Vec<String> = diverged
        ...
        .map(|p| {
            format!(
                "{} (was {})",
                p.branch,
                &p.expected_remote_sha.as_deref().unwrap_or("")[..7]
            )
        })
        .collect();
    let verb = if overwriting {
        "overwriting it"
    } else {
...
pub fn build_plan(input: PlanInput<'_>) -> Plan {
    ...
    for commit in input.commits {
        ...
        let action = match &pr {
            None => Action::Create,
            Some(p) if p.base_ref_name != previous_branch || p.title != title || p.body != body => {
                Action::Edit
            }
            Some(_) => Action::Keep,
        };
    ...
    let in_series: std::collections::HashSet<u64> = plan
        .entries
        .iter()
        .filter_map(|e| e.pr.as_ref().map(|p| p.number))
        .collect();
    ...
    for stack in input.stacks {
        if !stack
            .pull_requests
            .iter()
            .any(|p| in_series.contains(&p.number))
        {
            continue;
        }
    ...
    plan.superseded = input
        .prs
        .iter()
        .filter(|p| superseded_numbers.contains(&p.number))
        .cloned()
        .collect();
...
mod tests {
    ...
    fn orphans_are_open_prefixed_prs_outside_the_series() {
        ...
        assert_eq!(
            p.orphans.iter().map(|x| x.number).collect::<Vec<_>>(),
            vec![103]
        );
    }
    ...
    fn superseded_members_dissolve_only_their_stack() {
        let p = plan(
            ...
            &[pr(1), pr(2), pr(5)],
            &[
                stack(9, &[(101, "open"), (102, "open")]),
                stack(10, &[(105, "open"), (106, "open")]),
            ],
        );
        assert_eq!(
            p.superseded.iter().map(|x| x.number).collect::<Vec<_>>(),
            vec![102]
        );
    ...
    fn divergence_is_a_remote_tip_we_never_had() {
        let pushes = vec![
            Push {
                branch: "b1".into(),
                sha: "1".repeat(40),
                expected_remote_sha: Some("a".repeat(40)),
            },
            Push {
                branch: "b2".into(),
                sha: "2".repeat(40),
                expected_remote_sha: Some("f".repeat(40)),
            },
            Push {
                branch: "b3".into(),
                sha: "3".repeat(40),
