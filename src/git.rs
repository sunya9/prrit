...
impl<'a> Git<'a> {
    pub fn new(runner: &'a dyn Runner, cwd: &Path) -> Self {
        Git { runner, cwd: cwd.to_path_buf() }
    }

    fn git(&self, args: &[&str]) -> Result<String> {
        self.runner.run("git", args, &RunOptions { cwd: Some(&self.cwd), ..Default::default() })
    }

    fn git_with(&self, args: &[&str], input: &str, env: Vec<(&str, &str)>) -> Result<String> {
        self.runner.run("git", args, &RunOptions { cwd: Some(&self.cwd), input: Some(input), env, inherit: false })
    }
    ...
    pub fn default_branch(&self, remote: &str) -> Option<String> {
        ...
        let short = out.trim();
        Some(short.strip_prefix(&format!("{remote}/")).unwrap_or(short).to_string())
    }
    ...
    pub fn rev_list(&self, base: &str, head: &str) -> Result<Vec<RawCommit>> {
        ...
        for record in out.split('\0') {
            ...
            commits.push(RawCommit { sha, message, change_id });
        }
        Ok(commits)
    }
    ...
    pub fn ensure_change_ids(&self, commits: &[RawCommit], ref_name: &str) -> Result<EnsureResult> {
        if commits.iter().all(|c| c.change_id.is_some()) {
            let commits = commits
                .iter()
                .map(|c| Commit { sha: c.sha.clone(), message: c.message.clone(), change_id: c.change_id.clone().unwrap() })
                .collect();
            return Ok(EnsureResult { commits, rewritten: 0 });
        }
        ...
        for commit in commits {
            if let (Some(id), false) = (&commit.change_id, parent_changed) {
                result.push(Commit { sha: commit.sha.clone(), message: commit.message.clone(), change_id: id.clone() });
            ...
            result.push(Commit { sha: sha.clone(), message, change_id });
            parent = sha;
            parent_changed = true;
        }
        ...
        if parent_changed {
            ...
            self.git(&["update-ref", "-m", "prrit: add Change-Id", ref_name, &parent, &old_tip])?;
        }
        Ok(EnsureResult { commits: result, rewritten })
    }

    fn rewrite_commit(&self, sha: &str, parent: &str, message: &str) -> Result<String> {
        let tree = self.git(&["rev-parse", &format!("{sha}^{{tree}}")])?.trim().to_string();
        ...
        let out = self.git_with(
            ...
            message,
            vec![("GIT_AUTHOR_NAME", name), ("GIT_AUTHOR_EMAIL", email), ("GIT_AUTHOR_DATE", date)],
        )?;
    ...
    pub fn has_commit(&self, sha: &str) -> bool {
        self.git(&["cat-file", "-e", &format!("{sha}^{{commit}}")]).is_ok()
    }
    ...
    pub fn remote_url(&self, remote: &str) -> Option<String> {
        self.git(&["remote", "get-url", remote]).ok().map(|s| s.trim().to_string())
    }
...
mod tests {
    ...
    const RUNNER: ExecRunner = ExecRunner { inherit_target: InheritTarget::Stdout };
    ...
    fn rev_list_bottom_to_top_with_messages() {
        ...
        assert_eq!(commits.iter().map(|c| c.sha.clone()).collect::<Vec<_>>(), vec![a, b]);
    ...
    fn rev_list_rejects_merges() {
        ...
        sh(&repo.work, &["merge", "-q", "--no-ff", "-m", "merge side", "side"]);
        let git = Git::new(&RUNNER, &repo.work);
        assert!(git.rev_list("origin/main", "HEAD").unwrap_err().to_string().contains("merge commit"));
    }
    ...
    fn ensure_change_ids_rewrites_only_missing_keeping_tree_and_author() {
        ...
        let result = git.ensure_change_ids(&git.rev_list("origin/main", "HEAD").unwrap(), "HEAD").unwrap();
        ...
        assert_eq!(extract_change_id(&new_b.message).as_deref(), Some(new_b.change_id.as_str()));
        assert_eq!(sh(&repo.work, &["rev-parse", "HEAD"]), new_b.sha);
        assert_eq!(sh(&repo.work, &["rev-parse", "feature"]), new_b.sha);
        assert_eq!(sh(&repo.work, &["rev-parse", &format!("{}^{{tree}}", new_b.sha)]), tree_b);
        assert_eq!(
            sh(&repo.work, &["log", "-1", "--format=%an <%ae> %ai", &new_b.sha]),
    ...
    fn ensure_change_ids_rewrites_descendants() {
        ...
        assert_eq!(sh(&repo.work, &["rev-parse", &format!("{}^", result.commits[1].sha)]), result.commits[0].sha);
    }
    ...
    fn ensure_change_ids_moves_a_branch_that_is_not_checked_out() {
        ...
        assert_eq!(sh(&repo.work, &["rev-parse", "feature"]), result.commits[0].sha);
        assert_eq!(sh(&repo.work, &["rev-parse", "HEAD"]), sh(&repo.work, &["rev-parse", "origin/main"]));
    }
    ...
    fn ensure_change_ids_on_detached_head() {
        ...
        let git = Git::new(&RUNNER, &repo.work);
        let result = git.ensure_change_ids(&git.rev_list("origin/main", "HEAD").unwrap(), "HEAD").unwrap();
        assert_eq!(sh(&repo.work, &["rev-parse", "HEAD"]), result.commits[0].sha);
    }
    ...
    fn push_is_atomic_and_leased() {
        ...
        git.push(
            "origin",
            &[
                Push { branch: "prrit/alice/aaaaaaaa".into(), sha: a.clone(), expected_remote_sha: None },
                Push { branch: "prrit/alice/bbbbbbbb".into(), sha: b.clone(), expected_remote_sha: None },
            ],
        )
        .unwrap();
        ...
        sh(&repo.work, &["push", "-q", "-f", "origin", &format!("{b}:refs/heads/prrit/alice/aaaaaaaa")]);
        let stale = Push { branch: "prrit/alice/aaaaaaaa".into(), sha: a, expected_remote_sha: Some("0".repeat(40)) };
        assert!(git.push("origin", &[stale]).is_err());
        assert_eq!(git.ls_remote("origin", "prrit/alice/").unwrap()["prrit/alice/aaaaaaaa"], b);
